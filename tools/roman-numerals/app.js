/* ============================================================
   ...
   evident: the other two options are always "0" and "∞", visually
   marked (dashed border). Since Roman numerals have no symbol for
   zero or infinity, only the correct answer has a real symbol —
   the format makes the choice evident, without requiring the user
   to already know how to read the number to discard the other two.
...
(function () {
  ...
  function paintFamous() {
    ...
    /* The i18n sentence ends in ": <verb> <formula>". The formula
       is generated by this code from DATA.decompose(item.roman)
       so each digit carries the colour of the letter that
       originates it: 10 with X's colour, 5 with V's colour, 1
       with I's colour (red when it's the I that subtracts in
       IV/IX). That way the text and the operation are read
       together with the same colour code as the reminder. */
    var phrase = t('famous.' + item.factKey);
    var showFormula = item.showFormula !== false;
    if (showFormula) {
      /* Historical pattern: the i18n sentence ends with ":" and the
         coloured formula is appended after it ("10+10+1=21" for
         XXI). Each addend takes the colour of the letter that
         provides it. The prose before the colon is painted with
         the same colour code via paintFamousPhrase so the {X}/{x}
         placeholders inside it ("X vale 10", "la V vale 5") share
         the symbol → colour → value association. */
      ...
      /* The sentence already includes the numeric calculation
         ("…20+1=21") and we don't want to duplicate it by painting
         the coloured formula again. Even so, we strip a trailing
         ":" if there is one, so the sentence reads closed. The
         prose still goes through paintFamousPhrase so any {X}/{x}
         mention inside it ("X vale 10") keeps the colour code. */
      var cleaned = phrase.replace(/:\s*$/, '');
      famousText.innerHTML = paintFamousPhrase(item.roman, cleaned);
    }
  ...
  /* Returns the arithmetic formula of the Roman numeral with each
     ...
     mathematical notation. Examples:
       ...
       IV  → "5−1=4"
       VIII→ "5+1+1+1=8"
     Each addend keeps the colour of its letter (I→blue, V→green,
     X→orange, subtract→red) so the reader keeps associating
     symbol → colour → value; only the notation changes: instead of
     "10 plus 10, that is 20" it now reads "10+10=20". The formula
     is built with the same coloured tokens as the reminder
     (see decompositionFormula), changing only the connectors.
     The total at the end is wrapped in the same colour class as
     formatEquals uses in the reminder (see totalColorClass), so
     the formula closes with the colour of the letter that "owns"
     the value: orange for X (10), green for V (5), red for the
     I that subtracts in IV. */
  function coloredFormula(roman) {
    ...
    var sep = t('formulaSep');
    var parts = deco.tokens.map(function (tok) {
      var cls = 'letra-' + tok.letter.toLowerCase() + (tok.sign === '-' ? ' letra-resta' : '');
      return '<span class="' + cls + '">' + tok.value + '</span>';
    });
    var conj;
    if (parts.length === 1) {
      conj = parts[0];
    } else {
      /* Mathematical notation: each connector is "+" (no spaces) and
         the string ends with "=N" instead of ", that is N". */
      conj = parts.join(sep);
    }
    return conj + '=<span aria-hidden="true" class="' + totalCls + '">' + deco.total + '</span>';
  }
...
/* The default TTS would read "VI" as "uve-ié" or "XX" as separate
  ...
  /* Builds what's shown, the question, the 3 options (correct value
     plus the fixed "0"/"∞" decoys, per the module-wide "evident
     answer" format) and the real-life support sentence, based on the
     level's mode. hintRoman feeds the first-attempt Socratic hint;
     decoRoman is what's shown proactively in #decomposition (chips
     only — the total stays hidden until the round resolves, see
  ...
  function buildRound(entry) {
    ...
    var options = App.utils.shuffle([
      ...
      { value: '0', kind: 'no' },
      { value: t('infinityOption'), kind: 'inf' }
    ]);
    return {
  ...
  /* Text of the final formula. A single subtract-add pair uses its
     subtraction ("5 − 1"); a lone run (without anything else to
     combine it with) is expanded letter by letter ("10 + 10"); any
     other case uses the sum already grouped by chunks (e.g.
     "10 + 4" for XIV, without breaking the IV pair into
     10 − 1 + 5). */
  function decompositionFormula(deco) {
    if (deco.mode === 'subtract') return deco.subtract;
    if (deco.chunks.length === 1) return deco.tokens.map(function (t) { return t.value; }).join(' + ');
    return deco.sum;
  }
  ...
  function paintQuizDecomposition(roman, revealTotal) {
    ...
    if (hasOperation && revealTotal) {
      ...
      /* Total colour: if the decomposition is a subtract-add pair
         (subtract mode) the total is originated by the I that
         subtracts → red. Otherwise the colour of the first chunk
         (X→orange, V→green). Matches formatEquals in the reminder. */
      var totalClass = deco.mode === 'subtract'
        ? 'letra-i letra-resta'
        : 'letra-' + deco.chunks[0].letters[0].toLowerCase();
      formulaRow.innerHTML =
        '<span class="deco-formula-text">' + decompositionFormula(deco) + '</span>' +
        '<span class="deco-equals">=</span><span aria-hidden="true" class="deco-total ' + totalClass + '">' + deco.total + '</span>';
